...
const questionDifficultyOptions = ref([
  { value: 'easy', label: '简单' },
...
function getQuestionTypeLabel(type) {
  const option = questionTypeOptions.value.find(item => item.value === type)
  return option ? option.label : type
}

function getQuestionDifficultyLabel(difficulty) {
  const option = questionDifficultyOptions.value.find(item => item.value === difficulty)
...
function getList() {
  loading.value = true
  if (testId.value) {
    queryParams.testId = testId.value
  }
