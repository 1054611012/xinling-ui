  ...
  methods: {
    ...
    getList() {
      ...
      listInstruments(this.queryParams).then(response => {
        ...
        this.instrumentsList = this.instrumentsList.map(item => {
          return {
            ...item,
    ...
    handleUpdate(row) {
      ...
      getInstruments(id).then(response => {
        ...
        // 确保会员专属和是否显示字段是字符串类型
        this.form.isMemberOnly = String(this.form.isMemberOnly || "0")
        this.form.isDisplayed = String(this.form.isDisplayed || "1")
        this.open = true
        this.title = "修改乐器信息"
      })
    },
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateInstruments(this.form).then(response => {
              this.$modal.msgSuccess("修改成功")
              this.open = false
              this.getList()
            })
          } else {
            addInstruments(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除乐器信息编号为"' + ids + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('education/instruments/export', {
        ...this.queryParams
