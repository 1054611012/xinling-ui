...
function changeRadio(name, value) {
  ...
  if (arr.includes(name)) {
    if (value === "*") {
      ...
      isNaN(indexArr[0])
        ? (refName.cycle01 = 0)
        : (refName.cycle01 = indexArr[0])
      refName.cycle02 = indexArr[1]
      insValue = 2
    } else if (value.indexOf("/") > -1) {
      let indexArr = value.split("/")
      isNaN(indexArr[0])
        ? (refName.average01 = 0)
        : (refName.average01 = indexArr[0])
      refName.average02 = indexArr[1]
      insValue = 3
    } else {
      insValue = 4
      refName.checkboxList = value.split(",")
    }
  } else if (name == "day") {
    if (value === "*") {
      insValue = 1
